...
if st.session_state.get("I_done", False) or st.session_state.get("unlock_C_next", False):
    ...
    # ============================
    st.subheader("C1 — Included angle between Line 1 and Line 2")
    ...
    with c1a:
        gamma_guess = st.number_input(
            ...
            value=float(round(st.session_state.get("C_gamma_val", gamma_expected), 1)),
            step=0.5,
            key="C_gamma_guess"
        )
    with c1b:
    ...
    with c2a:
        F3_user = st.number_input(
            ...
            min_value=0.0,
            value=float(round(st.session_state.get("C_F3_val", F3_true), 2)),
            step=0.1,
            key="C_F3_user"
        )
    with c2b:
    ...
    if st.button("Check θ₃", key="C_btn_check_th3"):
        ...
        if abs(diff) <= tol_th3:
            ...
            st.session_state["C_theta3_guess"] = theta3_guess
        else:
            st.warning(
...
if st.session_state.get("C_done", False):
    ...
    st.markdown(
        ...
        "We’ll check equilibrium, compare with your earlier sign assumptions, "
        "and see if the magnitudes are reasonable."
    )
    ...
    theta3_student = st.session_state.get("C_theta3_guess", 0.0)  # deg, may be any range
    theta3_student_norm = theta3_student % 360.0
    th3r_student = math.radians(theta3_student_norm)
    F3x_student = F3_mag_student * math.cos(th3r_student)
    F3y_student = F3_mag_student * math.sin(th3r_student)

    # For reference, the "true" equilibrium F3 from components
    Sx = F1x + F2x
    Sy = F1y + F2y
    F3x_true = -Sx
    F3y_true = -Sy
    F3_true = math.hypot(F3x_true, F3y_true)
    th3_true = math.degrees(math.atan2(F3y_true, F3x_true))  # -180..180
    th3_true_360 = th3_true % 360.0

    st.markdown("### 1️⃣ Quick summary of your result vs reference")

    col_yours, col_ref = st.columns(2)
    with col_yours:
        st.markdown("**Your F₃**")
        st.write(f"|F₃| = {F3_mag_student:.2f} N")
        st.write(f"θ₃ (from +x) = {theta3_student_norm:.2f}°")
        st.write(f"F₃ₓ = {F3x_student:.2f} N")
        st.write(f"F₃ᵧ = {F3y_student:.2f} N")

    with col_ref:
        st.markdown("**Reference F₃ (from equilibrium components)**")
        st.write(f"|F₃| ≈ {F3_true:.2f} N")
        st.write(f"θ₃ ≈ {th3_true_360:.2f}° (0–360° form)")
        st.write(f"F₃ₓ ≈ {F3x_true:.2f} N")
        st.write(f"F₃ᵧ ≈ {F3y_true:.2f} N")

    st.markdown("### 2️⃣ Equilibrium check: does your F₃ balance F₁ and F₂?")

    tol_eq = st.number_input(
        "Equilibrium tolerance (N)",
        min_value=0.0, value=2.0, step=0.5,
        key="S_tol_eq"
    )

    if st.button("Check ΣFₓ and ΣFᵧ with my F₃", key="S_btn_check_eq"):
        sumFx = F1x + F2x + F3x_student
        sumFy = F1y + F2y + F3y_student

        st.markdown("#### Resultant with your F₃:")
        st.write(f"ΣFₓ = {sumFx:.2f} N")
        st.write(f"ΣFᵧ = {sumFy:.2f} N")

        ok_eq = (abs(sumFx) <= tol_eq) and (abs(sumFy) <= tol_eq)

        if ok_eq:
            st.success("✅ Your F₃ brings the system close to equilibrium (within tolerance).")
        else:
            st.warning(
                "⚠️ Your F₃ does **not** quite satisfy equilibrium with F₁ and F₂. "
                "Re-check either the magnitude, angle, or both."
            )

    st.markdown("### 3️⃣ Compare with your sign assumptions from the A step")

    assumed_F3x_pos = st.session_state.get("A_F3x_assume_saved", True)   # True = assumed +x
    assumed_F3y_pos = st.session_state.get("A_F3y_assume_saved", True)   # True = assumed +y

    actual_F3x_sign = "positive (+x)" if F3x_true > 0 else "negative (−x)"
    actual_F3y_sign = "positive (+y)" if F3y_true > 0 else "negative (−y)"

    assumed_F3x_str = "positive (+x)" if assumed_F3x_pos else "negative (−x)"
    assumed_F3y_str = "positive (+y)" if assumed_F3y_pos else "negative (−y)"

    with st.expander("Check sign consistency", expanded=False):
        st.write(f"**Your assumption in A:** F₃ₓ was {assumed_F3x_str}, F₃ᵧ was {assumed_F3y_str}.")
        st.write(f"**Actual from equilibrium:** F₃ₓ is {actual_F3x_sign}, F₃ᵧ is {actual_F3y_sign}.")

        ok_sign_x = ((F3x_true > 0) == assumed_F3x_pos)
        ok_sign_y = ((F3y_true > 0) == assumed_F3y_pos)

        if ok_sign_x and ok_sign_y:
            st.success(
                "✅ Your assumed directions for F₃ₓ and F₃ᵧ match the actual equilibrium directions. "
                "Nice intuition!"
            )
        else:
            msg = []
            if not ok_sign_x:
                msg.append("– F₃ₓ ended up opposite to what you assumed.")
            if not ok_sign_y:
                msg.append("– F₃ᵧ ended up opposite to what you assumed.")
            st.warning(
                "It’s OK if your initial assumption was wrong — the math tells you the correct direction.\n\n"
                + "\n".join(msg)
            )

    st.markdown("### 4️⃣ Magnitude reasonableness (triangle check)")

    # Triangle inequality sanity check
    # In any triangle: |F1 - F2| <= F3 <= F1 + F2
    lower_bound = abs(F1 - F2)
    upper_bound = F1 + F2

    st.write(
        f"For a valid triangle with sides |F₁|={F1:.1f} N and |F₂|={F2:.1f} N, "
        f"the third side must satisfy:  \n"
        f"**{lower_bound:.1f} N ≤ |F₃| ≤ {upper_bound:.1f} N**."
    )

    if lower_bound <= F3_mag_student <= upper_bound:
        st.success("✅ Your |F₃| lies in the valid triangle range — magnitude is geometrically reasonable.")
    else:
        st.warning(
            "⚠️ Your |F₃| does **not** satisfy the triangle inequality. "
            "Check if γ or your Law of Cosines step was done correctly."
        )

    st.success("🎓 S — Sanity check complete. You’ve walked through the full STATICS method on this problem.")
